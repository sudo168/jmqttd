...
/**
 * The simple implementation of B+-Tree.
 * reference:
...
public class BPlusTree <T extends Comparable<T>, V> {
     ...
     *
     * a) the factor must be greater than 2
     * b) the number of children for internal node must be in scope [ Math.ceil(factor/2), factor ].
     * c) the number of children for leaf node must be in scope [ Math.floor(factor-1/2), factor-1 ]
     */
    private int factor;
    ...
    private LeafNode<T, V> head = null;
    
    private LeafNode<T, V> tail = null;
    ...
    public BPlusTree(int factor) {
        ...
        this.MIN_FOR_LEAF = Double.valueOf(Math.floor(1.0 * this.factor - 1 / 2)).intValue();
    ...
    class LeafNode<T extends Comparable<T>, V> extends Node<T, V> {
        ...
        private LeafNode<T, V> next = null;
        ...
        public Node<T, V> insert(T key, V value) {
            ...
            for (; i < this.size; i++) {
                ...
                if (curKey.compareTo(key) == 0) {
                    this.values[i] = value;// 非unique key的时候，可能有多个值（多条记录）
                    return null;
                }
    ...
    public static void main(String[] args) {
        BPlusTree<Integer, String> myTree = new BPlusTree<>(64);
